...
1. **As specs são o contrato.** Se algo no código sugerir um caminho diferente do `design.md`, **pare e pergunte** (use `AskUserQuestion`) antes de divergir. Não decida sozinho.
...
5. **Siga as convenções do projeto.** Releia [AGENT.md](../../../AGENT.md) antes de codar. CSS Modules, Server Components por padrão, sem libs novas, etc.
6. **Verifique antes de declarar pronto.** A última fase do `tasks.md` é Verificação — `npm run lint`, `npm run build`, teste manual com `npm run dev` e `OpenPreview`. Nada de "deve funcionar".
...
2. **Implementar** a mudança (SearchReplace/Write nos arquivos certos).
3. **Verificar** localmente que não quebrou nada óbvio.
4. **Marcar o checkbox** no `tasks.md` (`- [ ]` → `- [x]`).
5. **Marcar o todo como `completed`.**

Se uma task se revelar inviável como descrita, **pare** e use `AskUserQuestion` para mostrar ao usuário o conflito antes de improvisar.
...
- `npm run dev` e `OpenPreview` — abrir o navegador, exercitar o caminho feliz e os edge cases listados no `design.md`.
- Conferir item por item os critérios de sucesso do `summary.md`.

### 6. Resumo final
Quando todos os checkboxes estiverem marcados e a verificação passar, responda ao usuário com o resumo do que foi feito.
